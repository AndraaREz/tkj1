...
local StarterGui = game:GetService("StarterGui") -- Tambahkan ini
...
function WaypointSystem:Initialize()
    ...
    self.waypointOrder = {}
    for waypointName, _ in pairs(self.waypoints) do
        table.insert(self.waypointOrder, waypointName)
    end
...
function WaypointSystem:SaveToFile()
    pcall(function()
        ...
        if success then
            -- Gunakan DataStoreService sebagai alternatif writefile
            local DataStoreService = game:GetService("DataStoreService")
            local waypointsDataStore = DataStoreService:GetDataStore("WaypointsData")
            waypointsDataStore:SetAsync(self.player.UserId .. "_waypoints", encodedData)
            print("✅ Waypoints saved to DataStore")
        else
            warn("❌ JSONEncode failed: " .. encodedData)
        end
    end)
end
...
function WaypointSystem:LoadWaypoints()
    pcall(function()
        -- Gunakan DataStoreService sebagai alternatif readfile
        local DataStoreService = game:GetService("DataStoreService")
        local waypointsDataStore = DataStoreService:GetDataStore("WaypointsData")
        local success, encodedData = pcall(waypointsDataStore.GetAsync, waypointsDataStore, self.player.UserId .. "_waypoints")
        if success and encodedData then
            local successDecode, data = pcall(HttpService.JSONDecode, HttpService, encodedData)
            if successDecode and data then
                for name, pos in pairs(data) do
                    self.waypoints[name] = Vector3.new(pos.X, pos.Y, pos.Z)
                end
                print("✅ Waypoints loaded from DataStore")
            else
                warn("❌ JSONDecode failed: " .. data)
            end
        else
            warn("❌ DataStore GetAsync failed: " .. encodedData)
        end
    end)
end
...
function WaypointSystem:CreateGUI()
    -- Periksa apakah GUI sudah ada
    local existingGui = self.player.PlayerGui:FindFirstChild("WaypointGUI")
    if existingGui then
        existingGui:Destroy()
    end
    ...
    gui.Parent = self.player.PlayerGui -- Pastikan parent sudah benar
    self.gui = gui
...
function WaypointSystem:SaveWaypoint()
    ...
    table.insert(self.waypointOrder, name) -- Tambahkan ke daftar urutan
    self:CreateWaypointItem(name)
    self:SaveToFile()
    self.textBox.Text = ""
    self:UpdateFooter()
...
function WaypointSystem:DeleteWaypoint(name, item)
    ...
    item:Destroy()
    -- Hapus item dari daftar urutan
     local index = table.find(self.waypointOrder, name)
    if index then
        table.remove(self.waypointOrder, index)
    end
    self:SaveToFile()
    self:UpdateFooter()
...
function WaypointSystem:ToggleMinimize()
    ...
    local targetSize = self.isMinimized and UDim2.new(0, 250, 0, 35) or UDim2.new(0, 250, 0, 400)
    ...
    -- Memastikan GUI tetap dapat digeser saat diminimalkan
    -- Tidak perlu koneksi tambahan di sini, koneksi sudah dibuat di Initialize dan SetupDragging
end
...
function WaypointSystem:CreateLoopSection(parent)
    local section = Instance.new("Frame")
    section.Name = "LoopSection"
    section.Size = UDim2.new(1, -20, 0, 45)
    section.Position = UDim2.new(0, 10, 1, -50)
    section.BackgroundTransparency = 1
    section.Parent = parent

    self:CreateToggle(section, "🔄 Loop Waypoints", function(enabled)
        self:SetLoopWaypoints(enabled)
    end)
end

function WaypointSystem:SetLoopWaypoints(enabled)
